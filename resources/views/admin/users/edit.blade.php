...
@section('title', 'Perbarui Akun Pengguna')

@section('content')
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="text-dark fw-800 mb-1"><i class="fa-solid fa-user-pen text-primary me-2"></i>Perbarui Akun Pengguna</h2>
            <p class="text-muted">Perbarui data profil kredensial serta tingkatan hak akses pengguna</p>
        </div>
    </div>
            ...
            <div class="glass-card">
                <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" id="name" class="form-control form-control-glass @error('name') is-invalid @enderror" value="{{ old('name', $user->name) }}" required placeholder="Nama lengkap">
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" name="email" id="email" class="form-control form-control-glass @error('email') is-invalid @enderror" value="{{ old('email', $user->email) }}" required placeholder="nama@email.com">
                        @error('email')
                            <div class="text-danger mt-1" style="font-size: 13px;">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="role" id="role" class="form-select form-control-glass @error('role') is-invalid @enderror" required>
                                ...
                                <option value="owner" {{ old('role', $user->role) == 'owner' ? 'selected' : '' }}>Owner (Pemilik UMKM)</option>
                                <option value="staff" {{ old('role', $user->role) == 'staff' ? 'selected' : '' }}>Staff (Kasir Toko)</option>
                            </select>
                            @error('role')
                                <div class="text-danger mt-1" style="font-size: 13px;">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="status" id="status" class="form-select form-control-glass @error('status') is-invalid @enderror" required>
                                ...
                                <option value="inactive" {{ old('status', $user->status) == 'inactive' ? 'selected' : '' }}>Inactive (Ditangguhkan)</option>
                            </select>
                            @error('status')
                                <div class="text-danger mt-1" style="font-size: 13px;">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="d-flex justify-content-end gap-3">
                        ...
                        <button type="submit" class="btn btn-glow-primary">Perbarui Pengguna</button>
                    </div>
                </form>
            </div>
        </div>
