...
@section('title', 'Daftar Mitra Usaha Baru')

@section('content')
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="text-dark fw-800 mb-1"><i class="fa-solid fa-plus text-primary me-2"></i>Registrasi Mitra Usaha</h2>
            <p class="text-muted">Buat profil unit usaha baru dan tetapkan penanggung jawab (Owner)</p>
        </div>
    </div>
                ...
                <form action="{{ route('admin.umkms.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="owner_id" id="owner_id" class="form-select form-control-glass @error('owner_id') is-invalid @enderror" required>
                            <option value="" disabled selected>Pilih user owner...</option>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name" id="name" class="form-control form-control-glass @error('name') is-invalid @enderror" value="{{ old('name') }}" required placeholder="Contoh: Butik Cantik Mode">
                            @error('name')
                                <div class="text-danger mt-1" style="font-size: 13px;">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="status" id="status" class="form-select form-control-glass @error('status') is-invalid @enderror" required>
                                ...
                                <option value="inactive" {{ old('status') == 'inactive' ? 'selected' : '' }}>Inactive (Dinonaktifkan)</option>
                            </select>
                            @error('status')
                                <div class="text-danger mt-1" style="font-size: 13px;">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="address" id="address" rows="3" class="form-control form-control-glass @error('address') is-invalid @enderror" required placeholder="Tuliskan alamat lengkap badan usaha mitra...">{{ old('address') }}</textarea>
                        @error('address')
                            <div class="text-danger mt-1" style="font-size: 13px;">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="2" class="form-control form-control-glass" placeholder="Keterangan atau profil singkat toko mitra...">{{ old('description') }}</textarea>
                    </div>

                    <div class="d-flex justify-content-end gap-3">
                        <a href="{{ route('admin.umkms.index') }}" class="btn btn-glow-outline">Batal</a>
                        <button type="submit" class="btn btn-glow-primary">Simpan Profil Mitra</button>
                    </div>
                </form>
            </div>
        </div>
